class ApisController < ApplicationController
  ...
  def lastfm_recommend_artists
    ...
    resp = HTTParty.post("http://ws.audioscrobbler.com/2.0/?method=user.getRecommendedArtists", body: {
        api_key: api_key,
        sk: sk,
        api_sig: api_sig
    })

    artists = resp['lfm']['recommendations']['artist'].shuffle.map {|artist| artist['name']}
